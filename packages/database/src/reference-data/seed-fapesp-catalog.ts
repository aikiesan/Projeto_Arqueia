...
import type { CP2bCatalogSeedResult } from './seed-cp2b-catalog.js';
...
export async function seedFapespReferenceCatalog(
  ...
  if (source === undefined) throw new Error('Não foi possível obter a fonte do catálogo FAPESP.');
  ...
  if (mismatchCount > 0) {
    throw new Error('A fonte FAPESP existente diverge do snapshot imutável. Crie uma nova versão.');
  }
